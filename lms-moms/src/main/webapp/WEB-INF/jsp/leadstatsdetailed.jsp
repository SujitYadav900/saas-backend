                                 ...
                                 <div class="table-responsive">
                                    <table class="table table-bordered table-striped table-hover">
                                       ...
                                       <tr>
                                          ...
                                          <th id="header2">Profiled</th>
                                          <th id="header3">Converted</th>
                                          <th id="header4">Rejected</th>
                                          <th id="header5">Pending</th>
                                       </tr>
                                       </thead>
                              ...
                              <div style="" class="panel-body">
                                 <form class="form-horizontal" id="filterform">
                                    <fieldset>
                                       ...
                                       <div class="col-md-10 form-group" id="clientTypeDiv" hidden >
...
function calculatePercentage(total,got) {
  ...
  function loaddefaultparams(){
      ...
      var  clientTypeFlag = searchParams.get("clientTypeFlag") || "none";
      let flag = searchParams.get("flag");
      let dateflag = searchParams.get("dateflag")||"multidate";

      //show clientType select
      if(flag == 'date' && dateflag == 'multidate'){
         $("#header3").html("Assessment Done");
         $("#header4").html("Assessment Pending");
         $("#header5").html("Enrolled");

         //$('#filtertab').trigger("click")//filter pannel
         $("#clientTypeDiv").show();
         let temp_url = "${pageContext.servletContext.contextPath}/api/leadtype/getactive"
         let temp_html = "<option value=''>---Select Lead Type---</option>";
         fetchselect(temp_url,temp_html);
      }
      ...
      var  url = "startdate="+startdate+"&enddate="+enddate+"&flag="+flag+"&dateflag="+dateflag+"&clientTypeFlag="+clientTypeFlag;
      ...
      $.ajax({
         ...
         success: function (json) {
            ...
            for(var i=0;i<json.length;i++)
            {
               ...
               rejectCount += parseInt(json[i].rejectCount)
               pendingCount += parseInt(json[i].pendingCount)
               ...
               if(flag == 'date'){
                  ...
                  userhtml += "<td hidval ='"+json[i].description+"' class='btn-link open-source-report' >"+description+"</td>";
               }else{
                  userhtml += "<td>"+description+"</td>";
               }

               userhtml += "<td>  <i title='Click Here To View Report' clienttype='"+json[i].clientType+"' hidid='"+json[i].seq+"' hidval ='"+json[i].description+"' filtertype = 'assigned' class='fa fa-table takemetotable'></i> "+json[i].assignedCount+"</td>";
               userhtml += "<td>  <i title='Click Here To View Report' clienttype='"+json[i].clientType+"' hidid='"+json[i].seq+"' hidval ='"+json[i].description+"' filtertype = 'profiled' class='fa fa-table takemetotable'></i> "+json[i].count+" ("+calculatePercentage(json[i].assignedCount,json[i].count)+")</td>";
               userhtml += "<td>  <i title='Click Here To View Report' clienttype='"+json[i].clientType+"' hidid='"+json[i].seq+"' hidval ='"+json[i].description+"' filtertype = 'converted' class='fa fa-table takemetotable'></i> "+json[i].convertCount+" ("+calculatePercentage(json[i].count,json[i].convertCount)+")</td>";
               userhtml += "<td>  <i title='Click Here To View Report' clienttype='"+json[i].clientType+"' hidid='"+json[i].seq+"' hidval ='"+json[i].description+"' filtertype = 'rejected' class='fa fa-table takemetotable'></i> "+json[i].rejectCount+" ("+calculatePercentage(json[i].count,json[i].rejectCount)+")</td>";
               userhtml += "<td>  <i title='Click Here To View Report' clienttype='"+json[i].clientType+"'  hidid='"+json[i].seq+"'  hidval ='"+json[i].description+"' filtertype = 'pending' class='fa fa-table takemetotable'></i> "+json[i].pendingCount+" ("+calculatePercentage(json[i].count,json[i].pendingCount)+")</td>";
               userhtml += "</tr>"
            }

            userhtml = userhtml+"<tr><td class='boldCell' > Total </td><td class='boldCell' >"+assignedCount+"</td><td class='boldCell' >"+userCount+"  ("+calculatePercentage(assignedCount,userCount)+")</td><td class='boldCell' >"+userConvertCount+"  ("+calculatePercentage(userCount,userConvertCount)+")</td><td class='boldCell' >"+rejectCount+" ("+calculatePercentage(userCount,rejectCount)+")</td><td class='boldCell' >"+pendingCount+" ("+calculatePercentage(userCount,pendingCount)+")</td></tr>"
...
//===========
$(document).on("click",".open-source-report",function () {

   let flag = "NONE";
   let dateflag = "multidate";

   //clientType or leadSource
   let hidval=$(this).closest("td").attr("hidval");
   //alert(hidval)

   //http://crm.momsbelief.com/mblms/leadstatsdetailed?&startdate=20210816&enddate=20210816&flag=source&dateflag=multidate&clientTypeFlag=NONE

   let  urlparams = "&startdate="+hidval+"&enddate="+hidval+"&flag=source&dateflag=multidate&clientTypeFlag=NONE"
   window.open("${pageContext.request.contextPath}/leadstatsdetailed?"+urlparams);
   ...
   $("#filterform").on("submit", function (e) {
      ...
      let clientTypeFlag = $("#clientTypeSelect").val() || "NONE";
      var urlparams=   "&startdate="+fromdate+"&enddate="+todate+"&flag="+flag+"&dateflag="+dateflag+"&clientTypeFlag="+clientTypeFlag;
   ...
   $(document).on("click",".takemetotable",function () {
      ...
      let leadpriority = 0;
      let leadType = 0;
      //1=leaddate,2=covertdate,3=updatedate,4=createdate,5=profillingdate,6=profilingdatecustom,8=assessmentdate
      let dateTypeFlag = 0;

      //LEAD STATS DATE WISE
      if(flag == 'date' && dateflag == 'multidate'){

         leadTypeTemp = $(this).attr("clienttype");
         if(leadTypeTemp == 'all'){
            leadType = 0;
         }else{
            leadType = leadTypeTemp;
         }
      ...
      // LEAD STATS SAME DATE
      else if(flag == 'date' && dateflag == 'createdate'){
         userfilter = hidid
         if(filtertype == "assigned"){
            leadstage = 0
            dateTypeFlag = 4;
      ...
      //LEAD STATS SOURCE WISE AND LEAD STATS USER WISE
      else {
         if(filtertype == 'assigned'){
            leadstage = 0
            dateTypeFlag = 4;
...
//http://localhost:8080/lead?datefiltertype=createdate&innersource=0&statusValue=0&leadSource=0&product=0&leadType=0&userFilter=6239&datefilter=false&datevalue=0&searchvalue=0&leadstage=0&leadpriority=0&fieldArray=
      var urlparams="statusValue="+leadstatus+"&leadSource="+leadsource+"&product="+0+"&leadType="+leadType+"&userFilter="+userfilter+"&datefilter="+datefilter+"&datevalue="+customdate+"&searchvalue="+0+"&leadstage="+leadstage+"&leadpriority="+leadpriority+"&keyword="+0+"&dateTypeFlag="+dateTypeFlag;
      window.open("${pageContext.request.contextPath}/lead?"+urlparams)

   })

   function fetchselect(url,temp_html) {
