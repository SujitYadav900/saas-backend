...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="header-icon">
                ...
                <div class="header-title">
                    ...
                    <a href="#" class="btn btn-add" data-toggle="modal" data-target="#addLeadType" title="Add new lead type"><i class="fa fa-plus"></i> Add Lead Type</a>
                </div>
        ...
        <section class="content">
                            ...
                            <div class="table-responsive">
                                <div class="table-responsive card" style="padding: 20px">
                                    <table class="table table-striped table-bordered first" id="table">
                                        <thead>
                                        <tr>
                                            <th>Name</th><th>Description</th><th>Post Call SMS</th><th>Status</th><th>Action</th>
                                        </tr>
                                        </thead>
                                        <tbody>
            ...
            <!--======================================================ADD NEW Lead Type=================================================================== -->
            <div class="modal fade" id="addLeadType" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header modal-header-primary">
                            <button type="button" onclick="clearData()" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                            <h3><i class="fa fa-plus m-r-5"></i> Add New Lead Type</h3>
                        </div>
                                    ...
                                    <form class="form-horizontal" id="createForm">
                                        <fieldset>
                                            ...
                                            <div class="col-md-10 form-group">
                                                <input type="checkbox" id="c2csms"  name="checkbox" /> Send SMS after call?
                                            ...
                                            <div class="col-md-8 form-group">
                                                ...
                                                <input type="text" maxlength="100" placeholder="Lead Type Name"  id="name" class="form-control" required>
                                            </div>
                                            <!-- Text input-->
                                            <div class="col-md-8 form-group">
                                                <label class="control-label">Description</label>
                                                <input type="text"  maxlength="256" placeholder="Lead Type Description"  id="description" class="form-control">
                                            </div>
                                    ...
                                    <form class="form-horizontal" id="addAgentForm">
                                        <fieldset>
                                            <div class="form-group col-md-12">
                                                <label>User </label>
                                                <select id="leadTypeSelect" name="leadType" required class="form-control">
                                            ...
                                            <div class="form-group col-md-12">
                                                <div class="table-responsive">
                                                    <table class="table table-bordered table-striped table-hover" id="usertable">
                                                        <tbody id="leadTypeAgentBody"></tbody>
                                                    </table>
                                                </div>
                                            </div>
...
<script>
    ...
    $(document).on('click','.addLeadAgentBtn',function () {
        ...
        var tempId = json.id;
        $('#leadTypeSelect').attr('leadtypeid',json.id);
        ...
        $.ajax({
            type: "GET",
            contentType: "application/json",
            url: '${pageContext.servletContext.contextPath}/api/leadtypeuser/getallbytypeid?lead_type_id='+tempId,
            success: function (json) {
                var html = "";
                for (var i = 0; i < json.length; i++) {
                    var html = $('#leadTypeAgentBody').html();
    ...
    $('#leadTypeSelect').on('change',function () {
        var userId = $(this).val();
        var username = $("#leadTypeSelect option:selected").text();
        var leadtypeid =$(this).attr('leadtypeid');
        ...
        if (userId == 0) {
            ...
            $.ajax({
                type: "POST",
                contentType: "application/json",
                url: "${pageContext.servletContext.contextPath}/api/leadtypeuser/insert",
    ...
    $(document).on('click','.deletUser',function(){
        ...
        swal({
            ...
            .then((willDelete) => {
                if(willDelete){
                    $.ajax({
                        url: "${pageContext.request.contextPath}/api/leadtypeuser/delete?id=" + id,
                        type: 'DELETE',
                        success: function () {
                            ths.remove();
                        },
...
//=================================== CREATE LEAD TYPE
    $('#createForm').submit(function (e) {
        ...
        if($('#createBtn').html() == 'Create'){
            ...
            var url = "${pageContext.request.contextPath}/api/leadtype/insert";
            let name= $('#name').val()
            name = name.trim();
            ...
            var map = {
                ...
                sendSms: $('#c2csms').is(":checked")
            };
        }

        else{

            var url = "${pageContext.request.contextPath}/api/leadtype/update";
            let name= $('#name').val()
            name = name.trim();
       ...
       $.ajax({
            ...
            if($('#createBtn').html() == 'Create'){
                ...
                swal({
                    title: "Success!",
                    text: "Successfully created Lead Type",
                    icon: "success",
                    button: "Ok!",
                });
                ...
                swal({
                    title: "Success!",
                    text: "Successfully Updated Lead Type",
                    icon: "success",
                    button: "Ok!",
                });
    ...
    var table=  $('#table').DataTable( {
        ajax: {
            url: '${pageContext.servletContext.contextPath}/api/leadtype/getall',
            dataSrc: '',
            "data": {
                "flag": 1,
        ...
        "columns": [
            ...
            { "data": "desc"},
            {
                "mRender": function (data, type, row) {
                    ...
                    if (row.sendSms == true) {
                        html = html + "<i style=' margin-left: 5px; font-size: 18px;color: greenyellow' title='Send SMS After Call' class=\"hvr-buzz-out fa fa-check\"></i>";
                    } else {

                        html = html + "<i style=' margin-left: 5px; font-size: 18px;color: red' title='Do not Send SMS After Call' class=\"hvr-buzz-out fa fa-ban\"></i>";
                    }
            ...
            { "mRender": function (data, type, row) {
                ...
                return '<button type="button" class="btn btn-add btn-sm editfield" title="Update" data-toggle="modal" data-target="#addLeadType" ><i class="fa fa-pencil"></i></button>' +'  '+
   ...
   function clearData()
   {
    ...
    $(document).on("click",".editfield",function () {
        ...
        $("#c2csms").prop('checked', json.sendSms);
