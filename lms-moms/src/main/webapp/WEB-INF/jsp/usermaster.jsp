    ...
    <div class="content-wrapper">
        ...
        <section class="content-header" id="addUserHeader">
            ...
            <div class="header-title" >
                <h1>User</h1>
                <button id="addnewuser" title="Add new user" class="btn btn-add">Add User <i class="hvr-buzz-out fa fa-plus"></i></button>
            </div>
        </section>
        <!-- Main content -->
        <div class="content">
            <div class="row">
                ...
                <div class="modal fade" id="viewLoginHistory" tabindex="-1" role="dialog" aria-hidden="true">
...
//================================================================== DATA TABLE ===========================================
    var table = $('#table').DataTable({
        ...
        "columns": [
            ...
            {"data": "dob"},
            {"data": "department"},
            {"data": "leadCountOfUser"},
            {"data": "leadCountOfReportTo"},
...

            {
                "mRender": function (data, type, row) {
                    ...
                    return "<button style='margin: 1px;' title='Click Here To Edit User' type=\"button\" class=\"edituser btn btn-add btn-sm\" > <i class=\"fa fa-pencil\"> </i> " +" "+
                        " <button style='margin: 1px;' data-toggle=\"modal\" data-target=\"#viewLoginHistory\" title='Click Here View Login History' type=\"button\" class=\"viewhistory btn btn-add btn-sm\" > <i class=\"fa fa-clock-o\"> </i>" ;
                }
            }
   ...
   //=======================================HIDE DATA TABLE==================================
    ...
    $(document).on("click", ".deletefile", function () {
        var hidid = $(this).closest("tr").find("td").eq(0).attr("hidatr");
        var ths = $(this);
        if (hidid == 0) {
            $(this).closest("tr").remove();
...
$(document).on("click",".viewhistory",function (){
      let json=JSON.parse($(this).closest("tr").find("td").eq(0).find("a").attr("hidatr"));

    $.ajax({
        url: "${pageContext.request.contextPath}/api/loginhistory/getlasttenhistory?userId="+json.id,
        type: 'GET',
        success: function (json) {
            var html = "";
            for(var i=0;i<json.length;i++){
