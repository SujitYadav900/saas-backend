...
@RestController
@RequestMapping("/api/combinedqueue2")
public class CombinedQueueMesasgeSendController {

	private final Logger logger = LoggerFactory.getLogger(CombinedQueueMesasgeSendController.class);

	@Autowired
	SessionRetrievalService sessionRetrievalService;
	@Autowired
	CombinedQueueMessageSendService combinedQueueMessageSendService;
	@Autowired
	...
	public ResponseEntity<MessageEntryResponseDao> MessageEntryResponseDaoPanel(@RequestBody List<MessageDao> messageDao, Authentication authentication) throws Exception {
		...
		logger.info("usermasterDao account ",+usermasterDao.getAccountId());
		long parentId = serviceImpl.getAdminAccount(usermasterDao.getAccountId());
		SubscriptionDao subscriptionDao = subscriptionService.getByActive(parentId);
	//	boolean enableAgentSupport = persistServiceImpl.getAgentSupportActive(parentId);
		if (Boolean.TRUE.equals(subscriptionDao.isEnableAgentSupport()) && parentId != 20004L) {
			logger.info("OutGoing coming from agent account ",+parentId);
			try {
			...
			messageEntryResponseDao = combinedQueueMessageSendService.messageBulkInsertService(messageDao, parentId,true);
			...
			logger.info("OutGoing coming from Admin account ",+usermasterDao.getAccountId());
			try {
				messageDao = prepareMessage(messageDao);
			} catch (ValidateFailedExeption ew) {
				logger.info("Validation Error occurs");
	...
	private TemplateMessageDao checkTemplate(TemplateMessageDao message) throws ValidateFailedExeption {
		...
		HashMap<String, CombinedQueueMesasgeSendController.ValidateTemplate> hashMap = new HashMap<>();
		...
		for (ComponentsDao componentsDao : message.getComponents()) {
			CombinedQueueMesasgeSendController.ValidateTemplate validateTemplate = hashMap.get(componentsDao.getType());
