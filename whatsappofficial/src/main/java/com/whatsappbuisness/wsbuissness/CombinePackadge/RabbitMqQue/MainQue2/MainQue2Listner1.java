package com.whatsappbuisness.wsbuissness.CombinePackadge.RabbitMqQue.MainQue2;
...
import com.whatsappbuisness.wsbuissness.CombinePackadge.RabbitMqQue.MainQue3.MainQue3Listner1;
...
@Component
public class MainQue2Listner1 {

    private static final Logger logger = LoggerFactory.getLogger(MainQue2Listner1.class);
    @Autowired
    ...
    @RabbitListener(queues = "Main_Que2", concurrency = "3")
    public void WebPanelQue(MessageDao messageDao, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) throws IOException {
        ...
        if (messageDao.getMessageStatus() == MessageStatus.PASSED) {
            if (messageDao.isPanel()) {
                logger.info("******* Will Push To Panel Que 2 *****");
                mqqueService.pushToQue(QueName.Contact_Que2.routing, QueName.Contact_Que2.queName, messageDao);
            } else {
...
//            commonClassReturnWithStatus = userOrBuisnessService.checkSessionWithCatagory(messageDao.getTo(), messageDao.getCountryCode(), messageDao.getAccountId(), messageDao.getChatSide(), messageDao.getCountryWiseRateRetDao().getCountryWisePriceDao().getBsnsInitiatedRates(), messageDao.getTemplate().getCategory());
        ...
        logger.info("commonClassReturnWithStatus {}", commonClassReturnWithStatus);
